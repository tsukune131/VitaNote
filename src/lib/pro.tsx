...
/**
 * 購入シートの価格表示を上書きする。ふだんは null にしておくこと。
 *
 * App内課金の「審査に関する情報」に添えるスクリーンショットには日本の価格
 * (¥500)が写っている必要があるが、価格はStoreKitがApple IDの国・地域を見て
 * 返すので、手元の端末が米国アカウントだと$2.99になり、日本の価格設定と
 * 食い違って見える。撮影のときだけここに '¥500' を入れて撮り、すぐ戻す。
 *
 * 上書きするのは表示だけで、実際の課金額はStoreKit側の値のまま。入れたまま
 * リリースすると表示と請求額が食い違うので、審査に出す前に必ず null に戻す。
 */
const PRICE_OVERRIDE: string | null = null;
